...
contract Ownable is Context {
    ...
    function unlock() public virtual {
        ...
        require(now > _lockTime , "Contract locked");
        emit OwnershipTransferred(_owner, _previousOwner);
        _owner = _previousOwner;
    }
}

contract CasperExpert is Context, IBEP20, Ownable {
    ...
    string  private _NAME = 'Casper Expert';
    string  private _SYMBOL = 'CSPRE';
    uint8   private _DECIMALS = 8;
   
    uint256 private constant _MAX = ~uint256(0);
    uint256 private _DECIMALFACTOR = 10 ** uint256(_DECIMALS);
    
    uint256 private _tTotal = 200000000 * _DECIMALFACTOR;
    uint256 private _rTotal = (_MAX - (_MAX % _tTotal));
    
    uint256 private _tFeeTotal;
    uint256 private _tBurnTotal;
    
    uint256 private _TAX_FEE = 250;
    uint256 private _BURN_FEE = 250;
    uint256 private _MAX_TX_SIZE = 30000000 * _DECIMALFACTOR;
    ...
    function _mint(address account, uint256 amount) internal {
        ...
        uint256 currentRate = _getRate();
        uint256 rMint = amount.mul(currentRate);
        _rOwned[account] = _rOwned[account].add(rMint);
        _rTotal = _rTotal.add(rMint);
